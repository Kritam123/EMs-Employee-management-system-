...
const Navbar = ({firstName,orgName,profile_img}:UserProfileProps) => {
  return (
    <>
    <Box className="fixed top-0 shadow-sm border-b  px-3 md:left-64 left-0 w-full md:w-[calc(100%-16rem)]">
        ...
        <div className="flex w-full h-14 justify-end items-center ">
                <Avatar className="cursor-pointer">
                    <AvatarFallback>{firstName && firstName[0]}-{firstName[firstName?.length-1]}</AvatarFallback>
                    <AvatarImage src={profile_img || featuredLogo}/>
                </Avatar>
        </div>
    </Box>
    </>
